...
;;(function( $,ebaui ) {
    ...
    ebaui.web.registerControl( 'TreeView' );

    /** 
     *  控件全名 e.g. ebaui.web.TreeView
     *  控件描述
     *  @class      ebaui.web.TreeView 
     *  @memberof   ebaui.web
     *  @extends    基类
     *  @tutorial   DEMO页
    ...
    ebaui.control( 'web.TreeView', {
        ...
        _parseUi : function( element ){
            ...
            */
            var me    = this;
            var $el   = $( element );
            var $html = $( '<ul class="ztree"></ul>' );
            $el.replaceWith( $html );
            /*
             *  如果是Form表单控件，一般会有name属性
             *  反之没有
             */
            var name = me.name ? me.name() : '';
            $html.attr( { 'data-name':name,'data-parsed': 'true' } );
         ...
         *  @method         _extendSelf
         */
        _extendSelf : function( source ){

            var me = this;
            for( var item in source ){

                var property = source[item];
        ...
        _initControl:function(){
            ...
            if( ds.length > 0 ){
                var $root = me._$root;
                var opts  = me.options['ztree'];
                var ins   = $.fn.zTree.init( $root, opts, me.dataSource());
                me._extendSelf( ins );
            }
        ...
        dataSource : function( val ){
            ...
            if( opts['dataSource'] != val ){
                opts['dataSource'] = val;
                $.fn.zTree.destroy(me.id());

                if( val.length > 0 ){
                    var ztree = opts['ztree'];
                    var ins   = $.fn.zTree.init(me._$root, ztree,val);
                    me._extendSelf( ins );
                }
    ...
    $(function(){ 
        $( 'input[data-role="treeview"]' ).treeview();
    });
